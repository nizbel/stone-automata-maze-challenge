...
class WalkingPathPuzzle:
    def __init__(self, current_position, puzzle, mark, conversions, path=None):
        # Position is a tuple with (row, col)
        self.current_pos = current_position
        # Keeps a list of moves (U, D, R or L)
        # self.path = [step for step in path]
    ...
    def __str__(self):
        return f'At {self.current_pos} on {self.conversions} conversions with path: {" ".join(self.path)}'
    ...
    def update_current_position(self):
        ...
        elif self.current_puzzle[x][y] in ['A', 'D'] and self.mark != self.current_puzzle[x][y]:
            self.current_puzzle[x][y] = self.mark
            self.conversions -= 1
            if self.conversions < 0:
                raise Exception('Used conversion without it being available')