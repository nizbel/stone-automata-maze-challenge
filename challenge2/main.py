    ...
    while not found_destination:
        ...
        for movement_index in reversed(range(len(possible_movements))):
            ...
            next_directions = {k: v for k, v in movement.get_possible_moves(map_rows, map_cols).items()
                               if (check_position_walkable(v) or movement.lives - 1 > 0)
            ...
            if next_directions:
                ...
                for direction, pos in next_directions[1:]:
                    ...
                    possible_movements.append(WalkingPath(pos, movement.path + [direction],
                                              movement.lives if check_position_walkable(pos) else movement.lives - 1))

                movement.walk(*next_directions[0], not check_position_walkable(next_directions[0][1]))

            else:
                # Remove path as it has nowhere to go
                del possible_movements[movement_index]

        # Sort movements to make sure the ones with more lives always come first
        # (first meaning being at the last position since we iterate in reversed)
        possible_movements.sort(key=lambda x: x.lives)
        ...
        for finished_move in [move for move in possible_movements
            ...
            print(finished_move)
            write_result_to_file(finished_move.path, 'output2.txt')
